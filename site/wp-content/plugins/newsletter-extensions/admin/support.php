...
if ($controls->is_action('send_support_data')) {
    if ($this->is_license_active()) {
        $response = wp_remote_post('https://www.thenewsletterplugin.com/wp-content/support-data.php?k='
                . urlencode(Newsletter::instance()->get_license_key()),
...
if ($controls->is_action('send_support_email')) {
    if ($this->is_license_active()) {
        $controls->errors = [];
        $message = NewsletterMailerAddon::get_test_message('test@thenewsletterplugin.com', 'Support test email from ' . home_url() . ' (' . date(DATE_ISO8601) . ')');
        ...
        $message = NewsletterMailerAddon::get_test_message('tnpplugin@gmail.com', 'Support test email from ' . home_url() . ' (' . date(DATE_ISO8601) . ')');
        ...
        $message = NewsletterMailerAddon::get_test_message('tnpplugin@yahoo.com', 'Support test email from ' . home_url() . ' (' . date(DATE_ISO8601) . ')');
        ...
        $message = NewsletterMailerAddon::get_test_message('tnpplugin@outlook.com', 'Support test email from ' . home_url() . ' (' . date(DATE_ISO8601) . ')');
...
<div class="wrap tnp-extensions-support" id="tnp-wrap">
    ...
    <div id="tnp-heading">
        <h2><?php esc_html_e('Support', 'newsletter') ?> <?php echo License::get_badge(); ?></h2>
    </div>
