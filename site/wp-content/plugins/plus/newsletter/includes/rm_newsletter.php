    ...
    public function subscribe($data,$list_id)
     {
        ...
        $email = $newsletter_subscription->normalize_email(stripslashes((string)$_REQUEST['ne']));
        $first_name = '';
        if (isset($_REQUEST['nn'])) {
            $first_name = $newsletter_subscription->normalize_name(stripslashes((string)$_REQUEST['nn']));
        }

        $last_name = '';
        if (isset($_REQUEST['ns'])) {
            $last_name = $newsletter_subscription->normalize_name(stripslashes((string)$_REQUEST['ns']));
        }

        $full_name = trim((string)$first_name . ' ' . $last_name);
