...
{
    if (!is_array($header)) {
        return array_map('trim', explode(',', (string)$header));
    }
...
function parse_request($message)
{
    ...
    $version = isset($parts[2]) ? explode('/', (string)$parts[2])[1] : '1.1';
...
function parse_query($str, $urlEncoding = true)
{
    ...
    foreach (explode('&', (string)$str) as $kvp) {
        $parts = explode('=', (string)$kvp, 2);
        $key = $decoder($parts[0]);
