...
if($data->selected_field=="Select"){
    $form->addElement(new Element_Textbox("<b>" . __('Default Select Label', 'registrationmagic-addon') . "</b>", "field_select_label", array("id" => "rm_field_select_label", "class" => "rm_static_field", "value" => empty($data->model->field_options->field_select_label) ? RM_UI_Strings::get('SELECT_FIELD_FIRST_OPTION') : $data->model->field_options->field_select_label, "longDesc"=>__("This will appear as the default selection text on the dropdown field.", 'registrationmagic-addon'))));
}
 ...
 jQuery(document).ready(function () {
   ...
   function rm_get_help_text(ftype){
        switch(ftype)
        {
            ...
            case "<?php echo $type; ?>":return "<?php echo addslashes(RM_UI_Strings::get("FIELD_HELP_TEXT_".$type)); ?>";        
            <?php } ?>
            default: return "<?php echo addslashes(RM_UI_Strings::get("HELP_ADD_FIELD_SELECT_TYPE")); ?>";
        }
    }
</script></pre>