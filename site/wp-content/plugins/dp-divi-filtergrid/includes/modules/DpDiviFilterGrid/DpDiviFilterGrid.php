...
class DpDiviFilterGrid extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		$layout_fields         = array(
			...
			'secondary_view'   => [
				...
				'description'     => __( 'Choose the order (drag and drop) of the elements and which elements to show (checkbox) in secondary view. Only elements turned on in Content tab will be available below.', 'dpdfg-dp-divi-filtergrid' ),
				'tab_slug'        => 'advanced',
				'toggle_slug'     => 'layout_options',
			],
		...
		return apply_filters( 'dpdfg_ext_get_fields', array_merge( $query_fields, $posts_elements_fields, $video_fields, $filters_fields, $sorting_fields, $url_options, $pagination_fields, $layout_fields, $popup_fields, $overlay_fields, $loaders_fields, $search_fields, $background_fields ) );
	}
	...
	public function before_render() {
		...
		*/
		...
		if ( strpos( $this->props['orderby_sort'], 'on|' ) || strpos( $this->props['orderby_sort'], '|on' ) ) {
			...
			$this->props['orderby_sort'] = json_encode( $sort_by_mig );
		}
		...
		if ( isset( $_SERVER['REQUEST_URI'] ) ) {
			...
			if ( 'on' === $props['url_navigation'] ) {
				...
				if ( 'on' === $props['show_filters'] && isset( $query_url_args['dfg_filter'] ) ) {
					$props['active_filter']      = $query_url_args['dfg_filter'];
					$props['use_taxonomy_terms'] = 'on';
					$active_filter               = explode( '|', $query_url_args['dfg_filter'] );
				}
				if ( 'on' === $props['show_sort'] && isset( $query_url_args['dfg_sort'] ) ) {
					$url_sort         = explode( '|', $query_url_args['dfg_sort'] );
					$props['order']   = $url_sort[0];
					$props['orderby'] = $url_sort[1];
				}
				if ( 'on' === $props['show_search'] && isset( $query_url_args['dfg_s'] ) ) {
					$props['query_var']['s'] = $query_url_args['dfg_s'];
				}
			}
		}
		// Init query
		...
		if ( isset( $posts_data['error'] ) ) {
			...
			if ( ! empty( $query_url_args ) ) {
				if ( isset( $query_url_args['dfg_active_filter'] ) ) {
					$active_filter_by_link = $query_url_args['dfg_active_filter'];
				}
				if ( $sfp_comp ) {
			...
			if ( $props['show_sort'] === 'on' ) {
				...
				$sort_output = sprintf( '<div class="dp-dfg-sorting%2$s">%1$s</div><!-- DPDFG End Sorting Container -->', $orderby_html . $order_html, $props['no_init'] === 'on' ? ' dp-dfg-hide' : '' );
			}
			// Filters
			$filters_output = '';
			if ( 'on' === $props['show_filters'] ) {
				...
				foreach ( $filters_levels as $filter_tax => $filter_level ) {
					...
					$level_active_filter = $active_filter;
					...
					switch ( $props['filter_layout'] ) {
						case 'button':
							...
							$buttons_output = '';
							foreach ( $filter_level as $filter ) {
								$buttons_output .= Dp_Dfg_Utils::filter_button_output( $filter, $level_active_filter, $props );
							}
	...
	public function get_data_module_for_ajax( $props ) {
