	...
	public static function get_items_html( $props, $posts ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( ! empty( $primary_view ) ) {
				...
				$final_output .= $sections_outputs['init'];
				if ( in_array( $props['items_skin'], $flip_skins ) ) {
					...
					$final_output .= sprintf( '<div class="dp-dfg-primary">%1$s</div><div class="dp-dfg-secondary">%2$s</div>', $side1, $side2 );
				} else {
					foreach ( $primary_view as $view ) {
						$final_output .= $sections_outputs[ $view->name ];
					}
				}
				$final_output .= $sections_outputs['end'];
			} else {
				$final_output = implode( '', $sections_outputs );
			}
