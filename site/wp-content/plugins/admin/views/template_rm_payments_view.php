...
$form_type_status = $data->form_type_status;
    ...
    ?>
    <input type="hidden" name="form_id" id="hd_form_id" value="<?php echo wp_kses_post((string)$data->form_id);?>">
    <input type="hidden" name="form_id" id="hd_submission_id" value="<?php echo wp_kses_post((string)$data->submission->submission_id);?>">
    <input type="hidden" name="form_id" id="hd_payment_id" value="<?php echo wp_kses_post((string)$data->payment->id);?>">
    <input type="hidden" name="form_id" id="hd_emaid_id" value="<?php echo wp_kses_post((string)$data->submission->user_email);?>">
    <div class="rmagic rm-payment-report-main">
        <div class="rm-box-title rm-box-mb-25 rm-payment-view-id"><?php _e('Payment ID ','registrationmagic-addon'); echo wp_kses_post((string)$data->payment->id);?></div>
        <!-- First Block-->
            ...
            <div class="rm-payments-header-rows rm-box-row rm-box-py-3">
                <div class="rm-payments-header-col rm-payments-header-column-1 rm-box-col-9">
                    <div class="rm-payments-header-top">
                        <div class="rm-payments-total">
                            <div class="rm-total-price"><?php _e('Total Price: ','registrationmagic-addon');?> <span><?php echo wp_kses_post((string)$data->payment->total_amount);?></span></div>
                                ...
                                <div class="rm-box-col-6">
                                    ...
                                    <div class="rm-payments-user-status rm-d-flex">
                                        ...
                                        if($form_type_status && !$user_type){
                                            echo '<div  class="rm-payments-value rm-di-flex rm-box-center rm-payment-user-activated">'.wp_kses_post((string)'Activated ','registrationmagic-addon').'</div>';
                                        }elseif (!$form_type_status && !$user_type) {
                                            echo '<div  class="rm-payments-value rm-di-flex rm-box-center rm-payment-user-existing">'.wp_kses_post((string)'Existing User','registrationmagic-addon').'</div>';        
                                        }
                                        elseif($user_type == 'guest'){
                                            echo '<div class="rm-payments-value rm-di-flex rm-box-center rm-payment-user-guest">'.wp_kses_post((string)'Guest','registrationmagic-addon').'</div>';
                                        }
                                        else{
                                            echo '<div class="rm-payments-value rm-di-flex rm-box-center rm-payment-user-pending">'.wp_kses_post((string)'Deactivated','registrationmagic-addon').'</div>';
                                        }
                                        ?>
                                    </div> 
                ...
                <div class="rm-payments-header-col rm-payments-header-column-2 rm-box-col-3">
                    ...
                    <div class="rm-box-btn-wrap rm-payments-actions rm-box-text-right">
                        <?php if($data->enable_invoice){?>
                            <div id="rm-payments-send-confirmation-email" class="rm-payment-action-btn" onclick="rm_send_payment_confirmation_email('<?php echo wp_kses_post((string)$data->submission->user_email);?>',<?php echo wp_kses_post((string)$data->form_id);?>,<?php echo wp_kses_post((string)$data->submission->submission_id);?>,<?php echo wp_kses_post((string)$data->payment->id);?>);"><a href="#"><button class="rm-btn-primary rm-btn"><?php _e('Resend Confirmation Email','registrationmagic-addon');?></button></a></div>
                        <?php }else{ ?>
