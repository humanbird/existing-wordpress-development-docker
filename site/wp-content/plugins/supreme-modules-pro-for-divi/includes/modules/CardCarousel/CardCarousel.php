...
class DSM_CardCarousel extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		if ( '' !== $arrows ) {
			$arrows_condition = 'off' === $arrows ? 'none' : 'block';
		...
		if ( '' !== $arrows_tablet ) {
			$arrows_tablet_condition = 'off' === $arrows_tablet ? 'none' : 'block';
			ET_Builder_Element::set_style(
				$render_slug,
				array(
					'selector'    => '%%order_class%% .dsm_card_carousel_arrow',
		...
		if ( '' !== $arrows_phone ) {
			$arrows_phone_condition = 'off' === $arrows_phone ? 'none' : 'block';
			ET_Builder_Element::set_style(
				$render_slug,
				array(
					'selector'    => '%%order_class%% .dsm_card_carousel_arrow',
		...
		et_pb_responsive_options()->generate_responsive_css( $image_width_values, $image_selector, 'max-width', $render_slug );
		...
		$image_alignment_selector = '%%order_class%% .dsm_card_carousel_child .dsm_card_carousel_child_img';

		if ( 'center' === $image_alignment ) {
			$image_alignment = 'auto';
		} elseif ( 'right' === $image_alignment ) {
			$image_alignment = 'auto 0 auto auto';
		} else {
			$image_alignment = 'auto auto auto 0';
		}

		if ( 'center' === $image_alignment_tablet ) {
			$image_alignment_tablet = 'auto';
		} elseif ( 'right' === $image_alignment_tablet ) {
			$image_alignment_tablet = 'auto 0 auto auto';
		} else {
			$image_alignment_tablet = 'auto auto auto 0';
		}

		if ( 'center' === $image_alignment_phone ) {
			$image_alignment_phone = 'auto';
		} elseif ( 'right' === $image_alignment_phone ) {
			$image_alignment_phone = 'auto 0 auto auto';
		} else {
			$image_alignment_phone = 'auto auto auto 0';
		}
