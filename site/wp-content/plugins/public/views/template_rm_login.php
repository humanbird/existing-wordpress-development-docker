...
if (!empty($data->show_otp))  // 2 Factor OTP form
{
    ...
    if(isset($data->design['placeholder_css'])){		
        $p_css = '<style>'.str_replace("::-", ' #'.$data->otp_form_slug.' ::-', wp_kses_post((string)$data->design['placeholder_css']));		
...
else // Normal form with username and password
{
    ...
    if(isset($data->design['placeholder_css'])){		
         $p_css = '<style>'.str_replace("::-", ' #'.$data->login_form_slug.' ::-', wp_kses_post((string)$data->design['placeholder_css']));		
