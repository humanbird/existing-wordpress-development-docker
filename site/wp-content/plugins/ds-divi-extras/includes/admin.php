...
// *** copied from: Extra/framework/admin/admin.php ***
...
class ET_Meta_Box {
	...
	function pre_register() {
		...
		// phpcs:disable WordPress.Security.NonceVerification -- no write operations being performed here
		if ( isset( $_GET['post'] ) ) {
	...
	function save_fields( $post_id ) {
		// Following nonce check was moved here from _save_post function to make phpcs happy :)
		...
		foreach ( $this->fields as $field_key => $field ) {
			...
			if ( isset( $_POST[ $field_key ] ) ) {
				if ( !empty( $field['value_type'] ) && 'array' == $field['value_type'] ) {
					...
					$value = implode( ',', array_map( $value_sanitize_function, wp_unslash( $_POST[ $field_key ] ) ) );
				} else if ( !empty( $field['value_type'] ) && 'checkbox' == $field['value_type'] ) {
					$value = 1;
				} else {
					// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- false positive
					$value = $value_sanitize_function( wp_unslash( $_POST[ $field_key ] ) );
				}
			}
...
// *** copied from Extra/includes/admin/admin.php ***
class AGSDCM_Extra_Page_Post_Settings_Meta_Box extends ET_Meta_Box {

	public function __construct() {
		parent::__construct( 'extra-page-post-settings', esc_html__( 'Divi Extras Settings', 'ds-divi-extras' ), array(
			'post_type' => array(
				'post',
			),
			'context'   => 'side',
	...
	function fields() {
		...
		//if ( 'post' == $post->post_type ) {
			$this->fields['_extra_featured_post'] = array(
				'title' => esc_html__( 'Featured Post', 'ds-divi-extras' ),
			);
		//}
	}
...
new AGSDCM_Extra_Page_Post_Settings_Meta_Box;
