...
function extra_text_module_layout_border_field( $field ) {
	...
	if ( EXTRA_LAYOUT_POST_TYPE == $post->post_type ) {
		$field['label'] = esc_html__( 'Show Top Border?', 'ds-divi-extras' );

		$field['options'] = array(
			'none' => esc_html__( 'No', 'ds-divi-extras' ),
			'top'  => esc_html__( 'Yes', 'ds-divi-extras' ),
		);

		$field['description'] = esc_html__( 'This will add a border to the top side of the module.', 'ds-divi-extras' );
	}
	return $field;
}
...
function extra_admin_bar_layout_edit( $wp_admin_bar ) {
	...
	$wp_admin_bar->add_node( array(
		'id'     => 'edit-layout',
		'parent' => false,
		'title'  => esc_html__( 'Edit Layout', 'ds-divi-extras' ),
...
function extra_layout_menu_home_layout_link() {
	...
	if ( !empty( $home_layout_id ) ) {
		$pagehook = add_submenu_page(
			...
			__( 'Edit Home Layout', 'ds-divi-extras' ),
			__( 'Edit Home Layout', 'ds-divi-extras' ),
			'edit_pages',
	...
	if ( !empty( $default_layout_id ) ) {
		$pagehook = add_submenu_page(
			'edit.php?post_type=' . EXTRA_LAYOUT_POST_TYPE,
			__( 'Edit Default Layout', 'ds-divi-extras' ),
			__( 'Edit Default Layout', 'ds-divi-extras' ),
			'edit_pages',
...
// Following function added to this file from core.php
// themes\Extra\includes\core.php
...
// themes\Extra\includes\template-tags.php
